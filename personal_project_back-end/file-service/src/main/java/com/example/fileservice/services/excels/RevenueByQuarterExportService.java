...
@Service
public class RevenueByQuarterExportService extends BaseExcelExportService<RoomRevenueReportModel> {
    @Override
    protected void writeHeader(Row headerRow) {
        String[] headers = {"Quý", "Năm", "Doanh thu"};
    ...
    protected void writeData(Sheet sheet, List<RoomRevenueReportModel> revenueReportModels) {
        ...
        for (RoomRevenueReportModel data : revenueReportModels) {
            ...
            row.createCell(0).setCellValue(data.getQuarter());
