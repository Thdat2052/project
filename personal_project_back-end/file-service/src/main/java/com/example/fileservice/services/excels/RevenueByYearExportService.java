...
@Service
public class RevenueByYearExportService extends BaseExcelExportService<RoomRevenueReportModel> {
    @Override
    ...
    protected void writeData(Sheet sheet, List<RoomRevenueReportModel> revenueReportModels) {
        ...
        for (RoomRevenueReportModel data : revenueReportModels) {
            ...
            row.createCell(0).setCellValue(data.getYear());

            Cell totalRevenueCell = row.createCell(1);
